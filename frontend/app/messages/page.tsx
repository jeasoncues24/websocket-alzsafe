...
function statusBadge(status: string, retryCount?: number) {
  switch (status) {
    case "sent":
      return <Badge variant="secondary">Enviado{retryCount ? ` (${retryCount})` : ""}</Badge>
    case "delivered":
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <CardHeader className="flex flex-col gap-4">
          ...
          <Tabs defaultValue="all" onValueChange={setActiveTab} className="flex flex-col gap-3">
        ...
        <CardContent className="flex flex-col gap-4">
          {!loading && messages.length === 0 ? (
            <DataEmptyState
              ...
              description="Prueba cambiando el filtro de empresa o estado."
            />
          ) : null}
          ...
          {loading || messages.length > 0 ? (
            <Table>
              ...
              <TableBody>
                {loading ? (
                  <TableLoadingRows columns={["w-12", "w-28", "w-28", "w-80", "w-24", "w-28"]} />
                ) : messages.map((msg) => {
                      ...
                      <TableCell>
                        <div className="flex justify-end gap-2">
                          ...
                          {msg.status === "failed" && msg.reference_id ? (
                            ...
                            >
                              {retryingId === msg.id ? (
                                <RefreshCw className="mr-2 h-4 w-4 animate-spin" data-icon="inline-start" />
                              ) : null}
                              Reintentar
                            </Button>
                          ) : null}
