...
func (s *AdminUserStore) Create(user *domain.AdminUser) (int64, error) {
	query := `INSERT INTO admin_users (username, password_hash, email, empresa_id, role, is_active) 
			  VALUES (?, ?, ?, ?, ?, ?)`

	result, err := s.db.Exec(query, user.Username, user.PasswordHash, user.Email,
		user.EmpresaID, user.Rol, user.Activo)
	if err != nil {
...
func (s *AdminUserStore) Update(user *domain.AdminUser) error {
	query := `UPDATE admin_users SET email = ?, empresa_id = ?, role = ?, is_active = ?, updated_at = NOW() 
			  WHERE id = ?`

	_, err := s.db.Exec(query, user.Email, user.EmpresaID, user.Rol, user.Activo, user.ID)
	if err != nil {
