...
func (s *UserModuleStore) GetByModuleID(moduleID int64) ([]domain.AdminUser, error) {
	query := `
		SELECT au.id, au.username, au.password_hash, au.email, au.role_id, au.activo, au.created_at, au.updated_at, au.last_login_at,
			   COALESCE(r.is_root, FALSE) as is_root
		FROM admin_users au
		JOIN user_modules um ON au.id = um.user_id
	...
	for rows.Next() {
		...
		err := rows.Scan(&u.ID, &u.Username, &u.PasswordHash, &email,
			&roleID, &u.Activo, &u.CreatedAt, &u.UpdatedAt, &lastLogin, &isRoot)
		if err != nil {
